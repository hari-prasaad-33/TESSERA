    ...
    <div className="relative flex h-full min-h-[26rem] flex-col justify-center overflow-hidden bg-[#0c1018]/90 p-6 sm:p-8 lg:p-10">
      ...
      <div className="relative z-10 grid gap-0">
        {stages.map((stage) => (
          <div
            key={stage}
            className="flex h-11 items-center border-b border-white/12 font-mono text-[10px] uppercase tracking-[0.24em] text-[#d5f8ff]"
          >
            {stage}
          </div>
        ))}
      ...
      <div className="w-full max-w-xl space-y-6">
        {tiers.map((tier) => (
          <div key={tier.title} className={`border-l-2 ${border[tier.accent]} pl-5`}>
            <div className="font-mono text-[10px] uppercase tracking-[0.26em] text-[#8d94ab]">{tier.title}</div>
            <div className="mt-2 flex flex-wrap items-baseline justify-between gap-2">
              <div className="text-2xl font-semibold tracking-[-0.04em] text-[#f0ebe0]">{tier.subtitle}</div>
              <div className="font-mono text-xs uppercase tracking-[0.24em] text-[#c6cfdd]">{tier.threshold}</div>
            </div>
          </div>
        ))}
      </div>
...
function BuildSlab({ id, title, subtitle, description, visual, reverse = false, slabIndex = 0 }) {
  return (
    <div className={slabIndex === 0 ? 'pb-2 pt-0' : 'border-t border-white/10 pb-2 pt-14'}>
      ...
      >
        <div className="flex flex-col justify-center px-0 py-2 sm:py-4 lg:px-2 lg:py-6 xl:pr-8">
          ...
          <p className="texture-type-shadow-soft mt-3 font-mono text-[10px] uppercase tracking-[0.26em] text-[#5dd4f0]">{subtitle}</p>
          <p className="texture-type-shadow-soft mt-6 max-w-xl text-base leading-relaxed text-[#c6cfdd]">{description}</p>
        </div>
      ...
      <div className="panel-shell">
        {includeIntro ? (
          <div className="mb-12 max-w-4xl lg:mb-16">
            ...
            <p className="texture-type-shadow-soft mt-6 max-w-2xl text-lg leading-relaxed text-[#c6cfdd]">
          ...
          <div className="mb-10 max-w-2xl">
            <p className="font-mono text-[10px] uppercase tracking-[0.32em] text-[#8d94ab]">Section 04 continued</p>
            <p className="texture-type-shadow-soft mt-2 text-sm text-[#c6cfdd]">Routing, confidence, and the DSP underneath.</p>
          </div>
        )}
